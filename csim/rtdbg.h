...
#define _DBG_LOG_HDR(lvl_name, color_n)                    \
    rt_kprintf("%s:%d\t\033["#color_n"m[" lvl_name "/" DBG_SECTION_NAME "] ", __FILE__, __LINE__)
...
/* Put fmt into __VA_ARGS__ so LOG_X("msg") is valid under ISO C++11
 * (empty ... after a separate fmt parameter is ill-formed / -Wpedantic). */
...
#define LOG_E_TAG()                                                             \
    do                                                                          \
    {                                                                           \
        rt_kprintf("%s:%d\t\033[31m[ EEEEEEEEE \033[0m\n", __FILE__, __LINE__); \
    } while (0)

#if (DBG_LEVEL >= DBG_LOG)
#define LOG_D_ARRAY(prefix, array, length)                           \
    do                                                               \
    {                                                                \
        uint32_t _len = length;                                      \
        uint8_t *_data = (uint8_t *)array;                           \
        LOG_RAW("%s:%d\t%s(%d) ", __FILE__, __LINE__, prefix, _len); \
        for (uint32_t _i = 0; _i < _len; _i++)                       \
        {                                                            \
            LOG_RAW("%02X", _data[_i]);                              \
        }                                                            \
        LOG_RAW("\n");                                               \
    } while (0)
#else
...
#if (DBG_LEVEL >= DBG_LOG)
#define LOG_C_ARRAY(prefix, array, length)                           \
    do                                                               \
    {                                                                \
        uint32_t _len = length;                                      \
        uint8_t *_data = (uint8_t *)array;                           \
